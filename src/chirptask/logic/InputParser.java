        ...
        switch (command) {
            ...
            case "addt" :
                ...
                if (dateList.size() == 2) {
                    ...
                    Task timed = new TimedTask(taskIndex, description,
                            startTime, endTime);
                    toDo = timed;
                } else {
                    return processInvalid(CommandType.ADD);
        ...
        if (!isIndexInRange(normalizedIndex)) {
            ...
            if (parameters.length > 1) {
                ...
                String toParse = getStringToParseDate(parameter,
                        oldTask.getType())[0];
                String newDesc = getStringToParseDate(parameter,
                        oldTask.getType())[1];
                ...
                if (newDesc == null || newDesc.equals("")) {
                    String oldNoDate = getStringToParseDate(
                            oldTask.getDescription(), oldTask.getType())[1];
                    editedTask.setDescription(oldNoDate);
                }
        ...
        switch (taskType) {
            ...
            case "timedtask" :
                ...
                if (editedDateList.size() > 1) {
                    ...
                    String start = new SimpleDateFormat("HH:mm")
                            .format(startDate.getTime());
                    String end = new SimpleDateFormat("HH:mm dd/MM")
